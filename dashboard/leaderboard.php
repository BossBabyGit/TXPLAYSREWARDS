...
// Handle form submission to update prizes
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Update prize amounts
    $prizeAmounts = [];
    for ($i = 1; $i <= 20; $i++) {
        $prizeKey = 'prize_' . $i;
        if (isset($_POST[$prizeKey])) {
            $prizeAmounts[$i] = (float)$_POST[$prizeKey];
        }
    }
    
    // Save to a file (in a real app, you'd save to database or config file)
    file_put_contents('prizes.json', json_encode($prizeAmounts));
    
    $success = "Prize amounts updated successfully!";
}

// Load current prize amounts
$prizeAmounts = json_decode(file_get_contents('prizes.json'), true) ?: [
    1 => 50000,
    2 => 24750,
    3 => 12500,
    4 => 8750,
...
<head>
    ...
    <title>TXPLAY - Leaderboard Management</title>
    <style>
        ...
        * {
        ...
        /* Add additional styles specific to leaderboard management */
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <div class="sidebar-icon active" onclick="window.location.href='leaderboard'">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="sidebar-icon" onclick="window.location.href='bonuses'">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="prizes-form">
                        <?php for ($i = 1; $i <= 20; $i++): ?>
                            <div class="form-group">
                                <label class="form-label" for="prize_<?php echo $i; ?>">Rank #<?php echo $i; ?> Prize</label>
                                <input type="number" id="prize_<?php echo $i; ?>" name="prize_<?php echo $i; ?>" 
                                       class="form-control" value="<?php echo htmlspecialchars($prizeAmounts[$i] ?? 0); ?>" 
            ...
            <div class="current-leaderboard">
                ...
                <table class="leaderboard-table">
                    ...
                    <tbody>
                        <?php for ($i = 1; $i <= 20; $i++): ?>
                            <tr>
                                <td class="rank-cell">#<?php echo $i; ?></td>
                                <td class="prize-cell">$<?php echo number_format($prizeAmounts[$i] ?? 0, 0, '.', ','); ?></td>
                            </tr>
                        <?php endfor; ?>
                    </tbody>
                </table>
